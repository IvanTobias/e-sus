...
from src.infra.db.repositories.sqls.units import UNITS
...
class CreateUnitsBaseRepository(CreateBasesRepositoryInterface):
    _base = 'unidades_saude'
    schema = {
            ...
            while _next:
                with DBConnectionHandler() as db:
                    ...
                    df = pd.read_sql_query(
                        text(f"{UNITS}  LIMIT {chunk_size} OFFSET {offset};"),
                        con=engine,
                    )
