...
class CreateEquipesBaseRepository(CreateBasesRepositoryInterface):
    _base = 'equipes'
    schema = {
        "cidadao_pec": pl.UInt64(),
        "codigo_unidade_saude": pl.UInt64(),
        "nome_unidade_saude": pl.String(),
        "codigo_equipe": pl.UInt64(),
        "nome_equipe": pl.String(),
        "ine": pl.String(),
        "micro_area": pl.String()
    }
            ...
            while _next:
                with DBConnectionHandler() as db:
                    ...
                    df = pd.read_sql_query(
                        text(f'{EQUIPES}  LIMIT {chunk_size} OFFSET {offset};'), con=engine)
